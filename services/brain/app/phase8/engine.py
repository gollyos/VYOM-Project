...
class Phase8Engine:
    """The advanced web-intelligence / professional-delivery operating
    layer. Mirrors BusinessEngine/IntelligenceEngine: a deterministic
    intent-to-workflow runtime the Task Runtime delegates to before
    reaching for a paid model. Research, browser, discovery, booking, and
    artifact capabilities are tools this engine composes for the Brain —
    VYOM does not become a browser wrapper."""
    ...
    async def _deep_research(self, task: Task, emit: EventEmitter) -> ExecutionResult:
        ...
        source_edges = [
            {"from": source.source_id, "to": claim.claim_id[:8]}
            for claim in result.claims for source in result.sources if source.source_id in claim.supporting_sources
        ]
        objects = [
            {
    ...
    async def _prepare_client_delivery(self, task: Task, emit: EventEmitter) -> ExecutionResult:
        ...
        except Exception as error:  # DuplicateDeliveryError or other guard
