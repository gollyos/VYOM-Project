...
class Phase10Engine:
    """Financial intelligence and controlled paper-trading operating
    layer. Mirrors Phase8Engine/Phase9Engine: a deterministic
    intent-to-workflow runtime the Task Runtime delegates to before
    reaching for a paid model. Nothing here places a real order — every
    execution path terminates at the local `PaperBroker`
    (docs/FINANCE_ARCHITECTURE.md, docs/TRADING_RISK_POLICY.md)."""
    ...
    async def execute(self, task: Task, profile: TaskProfile, emit: EventEmitter) -> ExecutionResult:
        ...
        raise RuntimeError(f"Unsupported Phase 10 intent: {intent}")
    ...
    async def _trade_postmortem(self, task: Task, emit: EventEmitter) -> ExecutionResult:
        ...
        # Separate outcome (loss/win) from process quality: a loss with a
        # sound, evidence-backed thesis is not automatically a bad decision
        # (rule 50).
        process_note = (
