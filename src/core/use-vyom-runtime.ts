...
type RuntimeSnapshot = {
  ...
  runCommand: (command: string, correlationId?: string) => void;
...
export function useVyomRuntime(): RuntimeSnapshot {
  ...
  const closeEverything = useCallback((command: string) => {
    clearTimers();
    lastCommandRef.current = command;
    setLastCommand(command);
    setOperationalMessage("Clearing contextual workspace locally");
    setResponse(null);
    setState("Executing");
    const hadComposition = Boolean(compositionRef.current);
    setComposerPhase(hadComposition ? "dismissing" : "calm");
    schedule(() => {
      setVisibleObjectIds([]);
      setComposition(null);
      ...
      setState("Verifying");
    }, hadComposition ? 680 : 260);
    schedule(() => {
      setState("Completed");
      setResponse("Everything is clear.");
      setOperationalMessage("Local deterministic command · no model call");
    }, hadComposition ? 1180 : 720);
    returnToCalm(hadComposition ? 2500 : 1900);
  }, [clearTimers, returnToCalm, schedule]);
  ...
  const submitToBrain = useCallback((command: string, correlationId?: string) => {
  ...
  const runCommand = useCallback((rawCommand: string, correlationId?: string) => {
    const command = rawCommand.trim();
    if (!command) return;
    trace(correlationId ?? newCorrelationId(), "command.received", { command });
    const intent = resolveIntent(command);
    if (intent === "close") return closeEverything(command);
    if (intent === "wake") {
      clearTimers();
      lastCommandRef.current = command;
      setLastCommand(command);
    ...
    submitToBrain(command, correlationId);
  }, [clearTimers, closeEverything, returnToCalm, schedule, submitToBrain]);
